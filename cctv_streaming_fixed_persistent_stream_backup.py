...
class OptimizedCCTVStream:
                ...
                try:
                    ...
                    if self.last_frame_time and (time.time() - self.last_frame_time) > 10:
                        ...
                        try:
                            ...
                            pass
                        try:
                            self.start_optimized_stream()
                        except Exception as e:
                            print(f"❌ 自動再起動失敗: {e}")
                        # 表示モードの復元（最小限）
                        try:
                            vm = int(self.ui_state.get('view_mode', 1))
                            self.change_view_mode(vm)
                        except Exception:
                            pass
                    time.sleep(5)
                except Exception as e:
                    print(f"❌ ウォッチドッグエラー: {e}")
                    time.sleep(5)
    ...
    def _create_optimized_session(self):
        ...
        # タイムアウト設定
        session.timeout = (2, 5)  # 接続タイムアウト2秒、読み取りタイムアウト5秒
    ...
    def get_specific_channels_frames(self, channel_list):
        ...
        if missing_channels:
            ...
            with ThreadPoolExecutor(max_workers=4) as executor:
                future_to_channel = {
                    executor.submit(self._get_channel_frame_fast, ch): ch 
    ...
    def start_optimized_stream(self):
        """最適化されたストリーム開始（持続ストリーム方式）"""
        # 持続ストリームを開始
        self.start_persistent_streams()
        self.is_streaming = True
        print("✅ 持続ストリーム方式でストリーミング開始")
        
        # すでにストリーミング中なら何もしない（冪等）
        if self.is_streaming:
            ...
            if self.last_frame_time and (time.time() - self.last_frame_time) > 5:
                ...
                print("ℹ️ すでにストリーミング中のため /start_stream は何もしません")
                return True
...
def index():
    ...
    <body>
        <div class="container">
            ...
            <div class="video-container" style="margin-bottom: 0;">
                <div class="video-section">
                    ...
                    <!-- 分割畫面 -->
                    <div id="gridView" class="grid-container grid-16" style="display: none; height: 500px;">
                        ...
                        <div class="grid-item" id="grid6">CH7</div>
                        <div class="grid-item" id="grid7">CH8</div>
                        <div class="grid-item" id="grid8">CH9</div>
                        <div class="grid-item" id="grid9" style="display: none;">CH10</div>
                        <div class="grid-item" id="grid10" style="display: none;">CH11</div>
                        <div class="grid-item" id="grid11" style="display: none;">CH12</div>
                        <div class="grid-item" id="grid12" style="display: none;">CH13</div>
                        <div class="grid-item" id="grid13" style="display: none;">CH14</div>
                        <div class="grid-item" id="grid14" style="display: none;">CH15</div>
                        <div class="grid-item" id="grid15" style="display: none;">CH16</div>
                    </div>
            ...
            <div class="view-controls">
                ...
                <button class="view-btn" onclick="changeView(16)" id="view16">16面</button>
        ...
        <script>
            ...
            let isCycling = false;
            // 循面用のチャンネルグループ定義
            const cycleGroupA = [3, 4, 7, 11];  // グループA: チャンネル3,4,7,11
            const cycleGroupB = [1, 5, 10, 13]; // グループB: チャンネル1,5,10,13
            ...
            function startCycle() {
                ...
                // 統合された循面更新タイマー（1秒間隔で更新、20秒間隔でグループ切り替え）
                window.cycleUpdateInterval = setInterval(() => {
                    ...
                    if (shouldSwitchGroup) {
                        ...
                        console.log('🔄 循面グループ切り替え:', currentGroup, '時刻:', new Date().toLocaleTimeString());
                    }
                    
                    // 現在のグループを更新（1秒間隔で滑らかな更新）
                    const currentGroup = cycleGroupIndex === 0 ? cycleGroupA : cycleGroupB;
                    console.log('🔄 循面1秒更新実行:', currentGroup, '時刻:', new Date().toLocaleTimeString());
                    displayCycleGroup(currentGroup);
                }, 1000);
                
                // 初期表示
                displayCycleGroup(cycleGroupA);
                updateStatus('🔄 循面グループ1開始: CH3,4,7,11', 'info');
            ...
            // 循面グループ表示関数（最適化版・デバッグ強化）
            function displayCycleGroup(channels) {
                const timestamp = new Date().toLocaleTimeString();
                console.log('🔄 循面更新開始:', channels, '時刻:', timestamp);
                
                // 4面表示のグリッドアイテムを取得
                const gridItems = document.querySelectorAll('.grid-item');
                
                // 4画面を一括取得（個別取得ではなく）
                const channelList = channels.slice(0, 4).join(',');
                console.log('🔄 循面API呼び出し:', '/get_multi_frames/4?channels=' + channelList);
                
                fetch('/get_multi_frames/4?channels=' + channelList)
                .then(response => response.json())
                .then(data => {
                    console.log('🔄 循面API応答:', data.success, 'フレーム数:', data.frames ? Object.keys(data.frames).length : 0);
                    
                    if (data.success && data.frames) {
                        // 各グリッドアイテムに映像を表示
                        channels.forEach((channel, index) => {
                            if (index < 4 && gridItems[index]) {
                                ...
                                if (data.frames[channelKey]) {
                                    ...
                                    console.log('⚠️ CH' + channel + ' フレーム無し');
                                }
                            }
                        });
                    } else {
                        console.log('❌ 循面一括取得失敗:', data.error || '不明なエラー');
                    }
                })
                .catch(error => {
                    console.log('❌ 循面通信エラー:', error.message);
                });
            }
...
def get_multi_frames(num_channels):
    ...
    try:
        ...
        # 循面用の複数チャンネル指定に対応: /get_multi_frames/4?channels=1,5,10,13
        ...
        if num_channels == 1 and channel_q:
            ...
            frames = cctv_system.get_specific_channels_frames(channel_list)
            detections = []
        else:
            # 単一モード解除
            try:
