...
# -*- coding: utf-8 -*-
"""
🚛 KIRII フォークリフト検出モデル - 低閾値動画テスト
============================================================
検出閾値を下げてフォークリフト検出を改善
"""
...
class ForkliftVideoTesterLowThreshold:
    def __init__(self, model_path="forklift_model.pt"):
        """フォークリフト動画テストシステム初期化（低閾値版）"""
        print("🚛 KIRII フォークリフト検出モデル - 低閾値テスト")
        print("=" * 60)
        ...
        # 検出設定（閾値を下げる）
        self.confidence_threshold = 0.1  # 10%に下げる
        self.iou_threshold = 0.3  # IOUも下げる
        
    def test_video(self, video_path):
        """動画ファイルでフォークリフト検出テスト（低閾値版）"""
        print(f"\n🎬 低閾値動画テスト開始: {video_path}")
        ...
        # 出力動画設定
        output_path = f"forklift_detection_low_threshold_{Path(video_path).stem}.mp4"
        ...
        while True:
            ...
            # 進捗表示
            if frame_count % 100 == 0:  # 100フレームごと
            ...
            threshold_text = f"Threshold: {self.confidence_threshold}"
            cv2.putText(annotated_frame, threshold_text, (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
...
def main():
    """メイン関数"""
    # フォークリフト動画テストシステム初期化（低閾値版）
    tester = ForkliftVideoTesterLowThreshold("forklift_model.pt")
